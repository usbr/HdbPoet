...
namespace HdbPoet.MetaData
{
    public class RatingTableData
    {
        ...
        public static string RatingSQL = " SELECT r.rating_id, indep_site_datatype_id, rating_type_common_name, "
            + "effective_start_date_time, effective_end_date_time, date_time_loaded, agen_id "//, description "
            + "FROM ref_site_rating r";
        
        public static string RatingViewSQL = "SELECT r.rating_id, rating_type_common_name, r.agen_id, s.site_name, indep_site_datatype_id, "
            ...
            + "AND d.unit_id = u.unit_id ORDER BY rating_id";

        public static string RatingtableListSQL = "AND rating_id = ";

        internal static DataTable RatingTableList(int rating_id = -1)
        {
            string sql = RatingViewSQL.Replace("ORDER BY rating_id", "");
            if (rating_id != -1)
            {
                sql += RatingtableListSQL + rating_id;
            }
